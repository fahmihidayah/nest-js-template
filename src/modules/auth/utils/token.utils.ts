...
export function createToken(user : User): TokenSerializer {
    ...
    return { expireIn : expires_in, 
        accessToken: sign(dataStoredInToken, secretKey ?? "test-1234", { expiresIn : expires_in }),
        refreshToken : sign(dataStoredInToken, refreshKey ?? "refresh-1234", { expiresIn : expires_in } )
    };
}
